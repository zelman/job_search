...
5. **PE acquirer list is incomplete.** ESW Capital (Influitive's acquirer Dec 2023) is not in the 38-firm `peFirms` list. Quick fix.
...
3. **Acquisition / PE ownership detection in Phase 1** — explicit `"<company> acquired by"` Brave query in last 24 months; expand `peFirms` list to include ESW Capital, Vista Equity, Thoma Bravo, KKR, Insight Partners (buyout mode), Francisco Partners, Symphony Technology Group, Jigsaw, similar rollup acquirers. If detected, force-route to PASS with Status "Acquired - Hard Pass".
