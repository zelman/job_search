...
2. `Enrich & Evaluate Pipeline v9.8.json` (company evaluation)
3. `Job Evaluation Pipeline v6.4.json` (job evaluation)
...
| # | Discrepancy | v9.8 Value | v6.4 Value | Recommended Resolution |
|---|-------------|------------|------------|------------------------|
| 1 | Employee hard cap | 350 | 200 | Align v6.4 to 350 |
| 2 | Employee soft cap | 200 | 150 | Align v6.4 to 200 |
| 3 | Funding hard cap | $450M | $500M | Align v6.4 to $450M |
...
| 7 | Unicorn valuation gate | >$1B = DQ | Missing | Add to v6.4 |
...
1. **Employee cap decision needed:** Should v6.4 use 350/200 (v9.8) or stay at 200/150? The higher caps catch VP roles at larger companies.

2. **Funding cap decision needed:** v6.4 uses $500M, v9.8 uses $450M. Which is authoritative?

3. **"tide_pool_scoring_adjustments.md" does not exist.** Prior session referenced values from this file ($75M funding DQ, 150 employee DQ) but file cannot be found. Were these from an earlier design that was superseded?

4. **Unicorn gate for jobs:** v9.8 has >$1B valuation = DQ. Should v6.4 also have this? (Jobs at unicorns may still be builder roles, unlike company-level alerts.)

5. **Network connection override feature:** v6.4 has hardcoded "Google" VP connection with override logic. Is this still needed? Should it be configurable?

---

## Recommended Next Steps

1. **Align v6.4 employee caps** to v9.8 (350 hard, 200 soft)
2. **Align v6.4 funding cap** to v9.8 ($450M)
3. **Port missing gates to v6.4:**
   - Known large companies list
   - Known acquired companies list
   - Stale funding check (3 years)
   - Unicorn valuation gate (if confirmed)
   - PLG-dominant gate
   - Explicit company age gate
4. **Document undocumented logic** (YC batch extraction, network override, JD signals)
5. **Confirm with Eric** on flagged decision items
