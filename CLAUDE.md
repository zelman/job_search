...
- `Indeed Job Scraper v5.json` - v5: **Builder-language query rotation** (Source Quality Pivot). Added 7 new queries targeting "founding", "first hire", "build", "ground up", "0 to 1" language in CS/Support job postings. Priority tiers: high (daily), medium (every 2 days), low (every 3 days). Query batching with 5s delay between searches. Tracks `_queryId`, `_isBuilderQuery` for signal analysis.
